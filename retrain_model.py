...
class Model:
    ...
    def predict_probability(self, pair, temp_df, value):
        prob = self.clf.predict_proba(temp_df)
        return prob[value]
    ...
    def retrain_strat(self, pair, temp_df):
        ...
        self.clf = DecisionTreeClassifier(self.parameters[pair], random_state = 123)
        self.clf.fit(data.loc[:,["MASlow", "MAFast", "Signal"]], data['Strategy Log Returns'].apply(create_target_feature))

        time_end = time.time()
        print(f"Time taken to retrain model for {pair}: {time_end - time_start} seconds")

        return True #best_parameters[pair]
